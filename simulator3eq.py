...
print '''
 ...
 ======================================
 Simulator: %s
 created by Leandro Marques at 22/2020
...
condition_xvelocity.gaussian_elimination(LHS_vx0,neighbors_nodes)
...
condition_yvelocity.gaussian_elimination(LHS_vy0,neighbors_nodes)
...
condition_streamfunction.streamfunction_condition(dirichlet_pts[3],LHS_psi0,neighbors_nodes)
# ---------------------------------------------------------------------------------
...
# ---------Boundaries conditions--------------------
bc = np.zeros([nphysical,1], dtype = float) 
...
xvelocity_bc_neumann,xvelocity_bc_dirichlet,xvelocity_LHS,xvelocity_bc_1,xvelocity_bc_2,xvelocity_ibc = b_bc(npoints, x, y, bc, neumann_edges[1], dirichlet_pts[1], xvelocity_LHS0, neighbors_nodes)
vorticity_ibc = xvelocity_ibc

# Applying vy condition
yvelocity_LHS0 = sps.lil_matrix.copy(M)
yvelocity_bc_neumann,yvelocity_bc_dirichlet,yvelocity_LHS,yvelocity_bc_1,yvelocity_bc_2,yvelocity_ibc = b_bc(npoints, x, y, bc, neumann_edges[2], dirichlet_pts[2],yvelocity_LHS0, neighbors_nodes)

# Applying psi condition (dirichlet condition only)
streamfunction_LHS0 = sps.lil_matrix.copy(K)
streamfunction_bc_1 = np.zeros([npoints,1], dtype = float) 
streamfunction_bc_neumann = np.zeros([npoints,1], dtype = float) 
streamfunction_ibc = []

for i in range(0, len(dirichlet_pts[3])):
 line = dirichlet_pts[3][i][0] - 1
 v1 = dirichlet_pts[3][i][1] - 1
 v2 = dirichlet_pts[3][i][2] - 1

 if line == 8:  
  streamfunction_bc_1[v1] = 0.0
  streamfunction_bc_1[v2] = 0.0

  streamfunction_ibc.append(v1)
  streamfunction_ibc.append(v2)

 if line == 11:  
  streamfunction_bc_1[v1] = 1.0
  streamfunction_bc_1[v2] = 1.0

  streamfunction_ibc.append(v1)
  streamfunction_ibc.append(v2)

 else:
  streamfunction_bc_1[v1] = y[v1]
  streamfunction_bc_1[v2] = y[v2]

  streamfunction_ibc.append(v1)
  streamfunction_ibc.append(v2)

streamfunction_ibc = np.unique(streamfunction_ibc)

# Gaussian elimination for psi
streamfunction_bc_dirichlet = np.zeros([npoints,1], dtype = float)
streamfunction_bc_2 = np.ones([npoints,1], dtype = float)
streamfunction_LHS = sps.lil_matrix.copy(streamfunction_LHS0)
for mm in streamfunction_ibc:
 for nn in neighbors_nodes[mm]:
  streamfunction_bc_dirichlet[nn] += float(streamfunction_LHS[nn,mm]*streamfunction_bc_1[mm])
  streamfunction_LHS[nn,mm] = 0.0
  streamfunction_LHS[mm,nn] = 0.0
   
 streamfunction_LHS[mm,mm] = 1.0
 streamfunction_bc_dirichlet[mm] = streamfunction_bc_1[mm]
 streamfunction_bc_2[mm] = 0.0


# ---------Initial condition--------------------
psi = streamfunction_bc_dirichlet
vx = xvelocity_bc_dirichlet
vy = yvelocity_bc_dirichlet
w = np.zeros([npoints,1], dtype = float)
...
streamfunction_RHS = np.multiply(streamfunction_RHS,streamfunction_bc_2)
streamfunction_RHS = streamfunction_RHS - streamfunction_bc_dirichlet

psi = scipy.sparse.linalg.cg(streamfunction_LHS,streamfunction_RHS,psi, maxiter=1.0e+05, tol=1.0e-05)
...
# w = w[0].reshape((len(w[0]),1))
 ...
 xvelocity_RHS = np.multiply(xvelocity_RHS,xvelocity_bc_2)
 xvelocity_RHS = xvelocity_RHS - xvelocity_bc_dirichlet
 vx = scipy.sparse.linalg.cg(xvelocity_LHS,xvelocity_RHS,vx, maxiter=1.0e+05, tol=1.0e-05)
 vx = vx[0].reshape((len(vx[0]),1))
 
 # Velocity vy
 yvelocity_RHS = -sps.lil_matrix.dot(Gx,psi)
 yvelocity_RHS = np.multiply(yvelocity_RHS,yvelocity_bc_2)
 yvelocity_RHS = yvelocity_RHS - yvelocity_bc_dirichlet
 vy = scipy.sparse.linalg.cg(yvelocity_LHS,yvelocity_RHS,vy, maxiter=1.0e+05, tol=1.0e-05)
