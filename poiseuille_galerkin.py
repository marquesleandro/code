...
for i in range(0, len(mesh.dirichlet_pts[1])):
 ...
 if line == 0 or line == 3:  
  ...
  bc_dirichlet_vx[v1] = ((4.0*v_max*mesh.y[v1])/L**2)*(L - mesh.y[v1])
  bc_dirichlet_vx[v2] = ((4.0*v_max*mesh.y[v2])/L**2)*(L - mesh.y[v2])
...
for i in range(0, len(mesh.dirichlet_pts[3])):
 ...
 if line == 11:  
  ...
  bc_dirichlet_psi[v1] = ((2.0*v_max*mesh.y[v1]**2)/(3.0*L**2))*(3.0*L - 2.0*mesh.y[v1])
  bc_dirichlet_psi[v2] = ((2.0*v_max*mesh.y[v2]**2)/(3.0*L**2))*(3.0*L - 2.0*mesh.y[v2])
...
for t in tqdm(range(0, nt)):
 ...
 save.saveVTK('./result_poiseuille_galerkin_100','parallel%s' %t)
